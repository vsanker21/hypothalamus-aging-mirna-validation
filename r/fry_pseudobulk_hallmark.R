# limma::fry on pseudobulk log-CPM (same design as camera), MSigDB Hallmark GMT.
# Exploratory: rotation/mixed gene-set test; complements camera (inter-gene correlation)
# and fgsea (rank-based); same GMT and pseudobulk normalization as camera_pseudobulk_hallmark.R.
#
# Usage (from feasibility_study/):
#   Rscript r/fry_pseudobulk_hallmark.R <counts.mtx> <rownames.csv> <colnames.csv> <metadata.csv> <hallmark.gmt> <out_csv>

args <- commandArgs(trailingOnly = TRUE)
if (length(args) < 6) {
  stop(
    "Usage: Rscript r/fry_pseudobulk_hallmark.R ",
...
fr <- fry(logcpm, idx, design, contrast = ncol(design))
out <- as.data.frame(fr)
out$pathway <- rownames(out)
rownames(out) <- NULL
utils::write.csv(out, out_csv, row.names = FALSE)
message("Wrote fry: ", normalizePath(out_csv, winslash = "/", mustWork = FALSE))

"""PROGENy pathway activities on GSE188646 pseudobulk counts via decoupler (academic license)."""
...
def run_progeny_on_pseudobulk(
    out_dir: Path,
    metadata_csv: Path | None = None,
    min_genes_per_pathway: int = 5,
) -> tuple[pd.DataFrame, pd.DataFrame | None] | None:
    """
    Pseudobulk counts are genes x samples (as written by pseudobulk_edgeR_gse188646.R).
    decoupler expects samples x genes.
    Returns (pathway_activity_scores, young_vs_aged_tests) or None if inputs missing.
    """
    ...
    # genes x samples -> log2-CPM; transpose to samples x genes for decoupler
    ...
    acts, pvals = dc.mt.mlm(mat, net, tmin=min_genes_per_pathway, verbose=False)
    acts = acts.copy()
    acts.insert(0, "sample", acts.index.astype(str))
    pvals = pvals.copy()
    pvals.insert(0, "sample", pvals.index.astype(str))
    ...
    if m_y.sum() >= 2 and m_a.sum() >= 2:
        ...
        for col in acts.columns:
            if col == "sample":
                continue
            v_y = acts.loc[m_y, col].astype(float)
            v_a = acts.loc[m_a, col].astype(float)
